...
Besides custom SQL, Workato also supports the execution of [stored procedures](/connectors/mssql/stored-procedure.md) that are defined in your database. Stored procedures similar to custom SQL are great ways for you to do some of the processing inside your databases before getting your recipe to retrieve the transformed data. Use stored procedures to join tables and perform complex queries before sending the result back to your recipe on Workato. Another great feature of stored procedures is the versatility of the procedure while still remaining easy to use for even the most basic users making recipes on Workato. By abstracting away the need for custom SQL and giving predefined input parameters, this balances flexibility with governance.
