...
1. Notifying your sales team in Slack about new leads on Salesforce
...
As a start, we would recommend shortlisting 4 or 5 objects which you want to interact with through a recipe in Workato. For smaller applications, this could mean all objects. For large applications with over hundreds of objects, this could mean shortlisting 5 of them which meet your integration use cases. Its completely fine to keep your scope small and iterate over time.
...
### Example 4:
As a developer tasked to build the connector to XYZ accounting, we can see that support should be prioritised for `Invoices`, `Products`, `Payments`, `Vendors` and `Expense reports`. When building a connector to your application for your customers, its common to extend the number of objects supported when needed to ensure that a larger number of customers can use your connector beyond the use cases you have defined.
...
Another possible type of action to support would be batch actions. Batch actions are actions that work with more than a single object but multiple ones and are often used in scenarios where multiple objects need to be synced in and out of an application. Consider if your users need this functionality to fulfil a core use case or if this can functionality added on later.
...
For some application, there may some special actions that aren’t included in your current shortlist of object-based actions and triggers. Review all integration use cases you and your team have set up to accomplish and take stock of any gaps in actions or triggers.
...
Using this skeleton, it’s easy to see that we’ve missed out an “Execute bank transfer” action which would be critical in this use case.
...
<table class="unchanged rich-diff-level-one">
    <thead>
        <tr>
          ...
          <th>New/updated object trigger
          </th>
          <th>Create object action
          </th>
          <th>Retrieve object action
          </th>
          <th>Update object action
          </th>
          <th>Delete object action
          </th>
          <th>Search object action
          </th>
          <th>Execute
          </th>
        </tr>
